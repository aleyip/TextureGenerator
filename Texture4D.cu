...
__global__ void raylightgenerator_kernel(T* out, int start, int* size, const T radius, const int wsTotal) {
	...
	if (ws == 1) {
		...
		if (size[2] != 1) angle[2] = asin(2 * s / (T)(size[2] - 1) - 1.);
		else angle[2] = 0;
		if (size[3] != 1) angle[3] = asin(2 * t / (T)(size[3] - 1) - 1.);
		else  angle[3] = 0;
	}
	else {
		angle[0] = M_PI * (2. * (n / (T)(ws - 1) - u - 0.5) / size[0] + 1);
		angle[1] = M_PI * ((v + m / (T)(ws - 1) - 0.5) / (size[1] - 1) - 0.5);
		if (size[2] != 1) angle[2] = asin(2 * (s + k / (T)(ws - 1) - 0.5) / (size[2] - 1) - 1);
		else angle[2] = 0;
		if (size[3] != 1) angle[3] = asin(2 * (t + l / (T)(ws - 1) - 0.5) / (size[3] - 1) - 1);
		else  angle[3] = 0;
	}
